<script lang="ts">
    ...
    let {
        initialFormat = "TRK-{YYYY}{MM}{DD}-{XXX}",
        onSave,
        onCancel,
        isSaving = false,
        currentPreviewTrackingCode = $bindable(""),
    } = $props<{
        initialFormat?: string;
        onSave: (format: string) => void;
        onCancel: () => void;
        isSaving?: boolean;
        currentPreviewTrackingCode?: string;
    }>();
    ...
    function removeComponent(id: string) {
        components = components.filter(
            (comp: FormatComponent) => comp.id !== id,
        );
    }
    ...
    const previewTrackingCode = $derived.by(() => {
        const now = new Date();
        const year = now.getFullYear().toString();
        const month = (now.getMonth() + 1).toString().padStart(2, "0");
        const day = now.getDate().toString().padStart(2, "0");
        let counter = 456; // Different from order number counter
    ...
    $effect(() => {
        currentPreviewTrackingCode = previewTrackingCode;
    });
    ...
    <div class="card-body p-6 lg:p-8">
        ...
        <div class="flex items-start justify-between mb-6">
            <div class="flex items-center gap-3">
                <div class="p-3 bg-accent/10 rounded-xl">
                    <QrCode class="h-6 w-6 text-accent" />
                </div>
                <div>
                    ...
                    <p class="text-sm text-base-content/60 mt-1">
                        Atur format unik untuk tracking code pengiriman
                    </p>
                </div>
            </div>
            {#if hasChanges}
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="bg-base-200/50 rounded-xl p-4 border border-base-300">
                ...
                >
                    ...
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        class="hover:bg-secondary/10 hover:text-secondary hover:border-secondary/50"
                    >
                    ...
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        class="hover:bg-accent/10 hover:text-accent hover:border-accent/50"
                    >
                        <Calendar class="h-4 w-4" />
                        Bulan
                    </Button>
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        class="hover:bg-info/10 hover:text-info hover:border-info/50"
                    >
                        <Calendar class="h-4 w-4" />
                        Hari
                    </Button>
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        class="hover:bg-success/10 hover:text-success hover:border-success/50"
                    >
                        <Hash class="h-4 w-4" />
                        Counter
                    </Button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                    class="bg-base-200/50 rounded-xl p-4 border border-base-300"
                >
                    <Label
                        for="format-string"
                ...
                <div class="bg-accent/5 rounded-xl p-4 border border-accent/20">
                    <Label
                        for="preview"
                        class="text-sm font-semibold text-accent mb-2 block"
                    >
                        <span class="flex items-center gap-2">
                            <div
                    ...
                    <Input
                        ...
                        readonly
                        class="font-mono text-lg font-bold bg-base-100 text-accent"
                    />
